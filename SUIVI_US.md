...
| US55 | Validation d'étape par QR Code         | ⚠️     | `StepValidationScreen` (`validationType='qrcode'`) : scanner `CameraView` fonctionnel, envoie `{ qr_code }`. ✅ `StepForm` backoffice permet maintenant de configurer `validation_type=qrcode` + `expected_code` (US23). Manque : validation côté serveur du code scanné vs `validation_data.expected_code`.               | 🟠 Moyenne |
| US56 | Validation d'étape par Quiz            | ⚠️     | `StepValidationScreen` (`validationType='quiz'`) : champ texte libre, validation insensible à la casse. ✅ `StepForm` backoffice permet maintenant de configurer `validation_type=quiz` + `correct_answer` (US23). Manque : QCM non supporté (spec prévoit liste de choix) ; validation serveur vs `validation_data.correct_answer`. | 🟠 Moyenne |
| US57 | Validation d'étape par photo           | ⚠️     | `StepValidationScreen` (`validationType='photo'`) : prise de photo avec `CameraView`, envoi de l'URI locale comme `file_url`. ✅ `StepForm` backoffice permet maintenant de configurer `validation_type=photo` (US23). Manque : upload photo vers `/files/upload` avant envoi de `file_url`.                               | 🟠 Moyenne |
...
| A4  | `StepEditor.tsx` (backoffice)             | ✅ **Résolu** — Stub supprimé (US23). `StepsPage` gère le CRUD via dialogs ; `HuntManager.tsx` n'y faisait pas référence.                                                                                                                                                                             | ✅ Résolu   |
